    ...
    return ClipRect(
      child: Stack(
        ...
        children: [
          ...
          if (image == null) ...[
            _Blob(top: -90,  right: -90, size: 280, color: const Color(0xFF4A90E2), opacity: isDark ? 0.28 : 0.13),
            _Blob(top: -70,  left: -50,  size: 340, color: const Color(0xFFA8C5FF), opacity: isDark ? 0.14 : 0.08),
            _Blob(bottom: -70, left: -70, size: 220, color: const Color(0xFFFF9F5C), opacity: isDark ? 0.18 : 0.09),
          ],
          Positioned.fill(
            child: DecoratedBox(
...
class _AvatarWidget extends StatelessWidget {
  const _AvatarWidget({required this.cs, required this.radius, required this.image, required this.onTap});
  final ColorScheme cs;
  final double radius;
  final File? image;
  final VoidCallback onTap;
        ...
        children: [
          Container(
            ...
            child: ClipOval(
              child: image != null
                  ? Image.file(image!, fit: BoxFit.cover)
                  : Icon(Icons.person_rounded, size: radius, color: cs.onPrimaryContainer),
            ),
          ),
          Positioned(
            bottom: 2, right: 2,
            child: Container(
              ...
              child: Icon(Icons.camera_alt_rounded, size: 11, color: cs.onPrimary),
            ),
          ),
        ],
      ),
